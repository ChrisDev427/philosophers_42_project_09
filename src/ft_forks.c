...
/*   Updated: 2023/05/04 19:09:34 by chmassa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_check_my_fork(t_philo  *philo)
{
    ...
    if (philo->fork == 1)
    {
        ...
        // printf(NBLUE"%ld %d has taken a fork\n"DEFAULT,ft_elapsed_time(philo->data->start_time, ft_get_time()), philo->id);
        ft_print(ft_elapsed_time(philo->data->start_time, ft_get_time()), philo->id, NBLUE"has taken a fork"DEFAULT);
